...
1. **严格遵守 JSON 格式**：输出必须是有效的 JSON，可以被 `json.loads()` 解析
2. **所有数组字段不能省略**：即使为空，也要输出 `[]`
3. **版本判断规则**：
   - 明确 V2 关键词（cToken, Comptroller, /v2/）→ "v2"
   - 明确 V3 关键词（Compound III, base asset, collateral asset）→ "v3"
   - 同时涉及两个版本或无法确定 → "unknown"
4. **中文回答**：`answer` 字段必须使用中文，清晰易懂
5. **引用完整性**：所有检索到的文档都要在 `sources` 中列出
